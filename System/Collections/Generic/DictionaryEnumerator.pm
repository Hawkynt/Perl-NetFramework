package System::Collections::Generic::DictionaryEnumerator; {
  ...
  # Enumerator for Generic Dictionary<TKey, TValue>
  sub new {
  ...
  sub MoveNext {
    ...
    if ($this->{_index} < scalar(@{$this->{_keys}})) {
      ...
      $this->{_current} = System::Collections::Generic::KeyValuePair->new(
        $bucket->{key}, 
        $bucket->{value}
      );
